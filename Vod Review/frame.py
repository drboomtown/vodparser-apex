...
class ApexVod():
    ...
    def main_vod(self, ref, frame_data, frame_count, health_bar_coord, filename, output, kill_only, merge, frame_skip,
        ...
        # starts a timer for how long it takes to proccess the video
        if debug is True:
            fps = FPS().start()
        ...
        # 90% of the time the fast option results are close enough and not noticable
        ...
        # if merge option is ticked in GUI, calls in the merge function and concatenantes all previously cut clips
        if merge is True:
